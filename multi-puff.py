...
#########################ENVELOP###############################
    def run_envelop(self, loc, height=1):
        ...
        self.loc = loc
        return self.run_gpu_contour(loc, height, True)
#########################END OF ENVELOP########################
...
#########################GIVEN_TIMES###########################
    def run_core_time(self, time_list, height_in=1):
        ...
        return self.run_gpu_contour(self, height_in, ticks=ticks)
#########################END OF GIVEN_TIMES#####################
            ...
            if cuda_disabled: #CPU Func
                ...
                for smoke in self.smoke_list:
                    ...
                    if not smoke.invalid and tick % OUTSTEP == 0:
                        ...
                        x, z = diffc; y = x
                        PI = math.pi
                        current_smoke_conc = ne.evaluate("mass/((2*PI)**1.5*x*y*z)*exp(-0.5*((CentralPositionMatrixX-X)/x)**2)*exp(-0.5*((CentralPositionMatrixY-Y)/y)**2)*(exp(-0.5*((Z-height_in)/z)**2) + exp(-0.5*((Z+height_in)/z)**2))")
                ...
                dest = run_gpu_conc(pos=numpy.array(pos).astype(numpy.float32), diffc=numpy.array(diffc).astype(numpy.float32), mass=numpy.array(mass).astype(numpy.float32),
                                            height=height_in, GRID_WIDTH=GRID_SIZE, gridw=GRID_INTERVAL, smoke_count=len(mass))
...
if __name__ == '__main__':
    ...
    if model.run_contour(height_in=2) == 0:
